	...
	return Entry.find().sort({timestamp: -1}).limit(3).exec(function(err, entries) {
		if (!err) {
			...
			var bgs = reversed.map(function(entry) {
				var delta = 0;
				if (prev) {
					delta = entry.bg - prev.bg;
				}
				prev = entry;
				return {datetime: new Date(entry.timestamp), sgv: entry.bg.toString(), trend: directionToTrend(entry.direction), direction: entry.direction, bgdelta: delta};
			}).reverse();
